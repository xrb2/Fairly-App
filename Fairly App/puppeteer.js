...
// Expanded product list (add more as needed)
...
async function fetchStorePrices(productName) {
  const searches = {
    ...
    // Add more stores (e.g., costco.com) here
  };
  ...
  const products = await Promise.all(
    pinkTaxProducts
      ...
      .map(async (product) => {
        ...
        const fairlyPrice = averagePrice > 0 ? (averagePrice * 0.9).toFixed(2) : 5.00; // 10% below average
